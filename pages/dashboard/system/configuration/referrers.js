...
export default function Referrers({ modals, setToken, setActiveDashboard, setActiveState, setActiveTab, setModalState, editFormState, getModalButtonRef, pageSelector, entryValue, searchField, resetSearchParams, setSearchParam }) {
    const [chargeView, setChargeView] = useState({})
    const [view, setView] = useState(false)
    const [referralData, setReferralData] = useState()
    const fetching = (url) => axios.get(url, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }).then(res => res.data)
    const { data: referrerApiData, error: referrerApiDataError } = useSWR(`${testEnv}v1/referrer/all?pageNo=0&pageSize=10`, fetching)
    const { data: searchBarData, error: searchBarDataError } = useSWR(`${testEnv}v1/referrer/search?pattern=${searchField}&pageNo=${entryValue.page}&pageSize=${entryValue.size}`, fetching)
    ...
    useEffect(() => {
        ...
        setActiveState("1")
        if (referrerApiData) {
            setReferralData(referrerApiData)
        }
    }, [referrerApiData])
        ...
        <div className="">
            <section className={`px-[40px] mdxl:px-[10px] pt-2 pb-2 w-fit md:w-full mt-8 h-fit lg:h-[61px] flex flex-col mdxl:flex-row justify-between items-center rounded-[48px] bg-[#F3F3F3] md:pr-[60px]`}>
                <section className="w-[354px] h-[40px] bg-white rounded-[20px] px-2 relative flex items-center justify-between">
                    <input onChange={(e) => { setSearchParam(e) }} className="search-tab rounded-[20px] w-[80%]" placeholder="Search referral" />
