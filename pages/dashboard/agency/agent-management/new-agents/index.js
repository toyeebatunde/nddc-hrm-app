...
export default function Agents({ modals, setToken, setActiveDashboard, setActiveState, entryValue, pageSelector, setActiveTab, dateRange, search, setSearch, setLoading }) {
    ...
    useEffect(() => {
        ...
        // }
        mutate(`${testEnv}v1/agent/new_agents/filter_all_by_dates?from=${formatDate(dateRange.dateFrom)}&pageNo=${entryValue.page}&pageSize=${entryValue.size}&to=${formatDate(dateRange.dateTo)}`)
        if (dateFiltered) {
