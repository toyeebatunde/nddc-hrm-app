        ...
        <div id='container' className='mt-[60px] flex flex-col w-full lg:w-[529px] min-h-[550px] mb-[50px] border border-border-gray rounded-[48px]'>
          ...
          <section className='flex flex-col'>
            ...
            <section className='w-[90%] md:w-[427px] mt-[20px] h-[57px] relative justify-between bg-[#f9f9f9] pr-6 border m-auto border-border-gray rounded-[28.5px] flex items-center'>
              <input value={''} onChange={(e) => { changeForm(e, signupDetails, setSignupDetails) }} id='username' className='ml-10 border-0 bg-input-gray w-4/5 focus:border-none outline-none' type="text" placeholder='Username' />
            </section>
            <section className='w-[90%] md:w-[427px] mt-[20px] h-[57px] relative justify-between bg-[#f9f9f9] pr-6 border m-auto border-border-gray rounded-[28.5px] flex items-center'>
              <input value={''} onChange={(e) => { changeForm(e, signupDetails, setSignupDetails) }} id='email' className='ml-10 border-0 bg-input-gray w-4/5 focus:border-none outline-none' type="email" placeholder='Email' />
            </section>
            <section className='w-[90%] md:w-[427px] mt-[20px] h-[57px] relative justify-between bg-[#f9f9f9] pr-6 border m-auto border-border-gray rounded-[28.5px] flex items-center'>
              <input value={''} onChange={(e) => { changeForm(e, signupDetails, setSignupDetails) }} id='phone-number' className='ml-10 border-0 bg-input-gray w-4/5 focus:border-none outline-none' type="tel" placeholder='Phone Number' />
            </section>
            <section className='w-[90%] md:w-[427px] mt-[20px] h-[57px] relative justify-between bg-[#f9f9f9] pr-6 border m-auto border-border-gray rounded-[28.5px] flex items-center'>
              <input value={''} onChange={(e) => { changeForm(e, signupDetails, setSignupDetails) }} id='password' ref={passwordField} className='ml-10 w-4/6 z-10 focus:border-none outline-none bg-input-gray' type={passwordDisplay.password} placeholder='Password' />
          ...
          <section className='mt-[30px] gap-[20px] lg:gap-0 m-auto w-[90%] md:w-[425px] flex items-center justify-between'>
            <section className='font-pushpennyBook text-[12px] leading-[15.62px]'>
              Have an account already? <span onClick={()=>{router.push("/login")}} className='underline cursor-pointer sec-color'> Login instead </span></section>
            <button className='bg-gradient-to-r from-[#003B49] to-[#2DCD7C] active:bg-white active:text-[#2DCD7C] w-[126px] h-[46px] font-[400] text-[#ffffff] rounded-[23px]'>{isLoading ? "Signing you up..." : "Sign up"}</button>
          </section>
        </div>
      </div>
    </div>
