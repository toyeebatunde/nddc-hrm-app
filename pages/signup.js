...
export default function Home({ showPassword, login, isLoading, token, passwordDisplay, setPasswordDisplay, changeForm, signupDetails, setSignupDetails, createCaution, changer }) {
  ...
  async function signup(e) {
    ...
    if (!validatePassword(loginDetails.passwordOne)) {
      setPasswordCheck("Password must be up to 8 characters and contain at least 1 uppercase letter, 1 lowercase letter, 1 special character and 1 number");
      return;
    }
  ...
  function handleFormChange(e) {
    ...
    if (e.target.name == "number") {
      if ((!Number(e.target.value + 1) && (e.target.value != "")) || (e.target.value.length == 12)) {
        return
      }
    }
        ...
        <div id='container' className='mt-[60px] flex flex-col w-full lg:w-[529px] min-h-[550px] mb-[50px] border border-border-gray rounded-[48px]'>
          ...
          <section className='flex flex-col'>
            ...
            <section className='w-[90%] md:w-[427px] mt-[20px] h-[57px] relative justify-between bg-[#f9f9f9] pr-6 border m-auto border-border-gray rounded-[28.5px] flex items-center'>
              <input value={loginDetails.number} onChange={(e) => { handleFormChange(e) }} id='phone-number' name='number' className='ml-10 border-0 bg-input-gray w-4/5 focus:border-none outline-none' type="tel" placeholder='Phone Number' />
            </section>
            <section className='w-[90%] md:w-[427px] mt-[20px] h-[57px] relative justify-between bg-[#f9f9f9] pr-6 border m-auto border-border-gray rounded-[28.5px] flex items-center'>
              <input value={loginDetails.passwordOne} name='passwordOne' onChange={(e) => { handleFormChange(e) }} id='password' ref={passwordField} className='ml-10 w-4/6 z-10 focus:border-none outline-none bg-input-gray' type={passwordDisplay.password} placeholder='Password' />
            ...
            <section className='w-[90%] md:w-[427px] mt-[20px] h-[57px] relative justify-between bg-[#f9f9f9] pr-6 border m-auto border-border-gray rounded-[28.5px] flex items-center'>
              <input value={loginDetails.passwordTwo} name='passwordTwo' onChange={(e) => { handleFormChange(e) }} id='password' ref={passwordField} className='ml-10 w-4/6 z-10 focus:border-none outline-none bg-input-gray' type={passwordDisplay.password} placeholder='Password' />
              <button onClick={() => { showPassword(passwordField, setPasswordDisplay, passwordDisplay) }} className='bg-input-gray z-30 px-3 ml-auto cursor-pointer rounded-[10px]'>
                {passwordDisplay.password == "password" ? "Show" : "Hide"}
              </button>
            </section>
