        ...
        <div className={`flex relative flex-col items-start pt-[60px] w-full`}>
            ...
            <section className={`h-[44px] flex flex-col w-full px-4 relative mt-5 ${modals.isOpen ? "blur-sm" : "blur-none"}`}>
                <div className={`flex h-[44px] absolute z-[40] w-full ${activeAgency == "Configurations" ? "justify-start gap-[50px]" : "justify-start gap-[100px]"} relative`}>
                    {activeAgency == "Configurations" ? tabs.map((tab, index) => <div key={index}><ButtonTab tabKey={index} name={tab.name} url={tab.url} activeTab={activeTab} link={true} setTab={setActiveTab} /></div>) : activeAgency == "UserManagement" ? userTabs.map((tab, index) => <div key={index}><ButtonTab tabKey={index} name={tab.name} url={tab.url} activeTab={activeTab} link={true} setTab={setActiveTab} /></div>) : null }
