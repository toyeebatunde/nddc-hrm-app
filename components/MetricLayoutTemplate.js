...
export default function MetricLayoutTemplate({ children, title, modals, activeAgency, viewState, setView }) {
    ...
    return (
        <div className="flex flex-col items-center pt-[60px] w-full">
            ...
            <section className={`h-[44px] ${activeAgency == "Agent Management" ? "flex" : "hidden"} flex-col w-full px-4 relative mt-5 ${modals.isOpen ? "blur-sm" : "blur-none"}`}>
            ...
            <section className={`px-4 flex justify-center w-full ${modals.isOpen ? "blur-sm" : "blur-none"}`}>
                <section className={`px-[40px] mdxl:px-[10px] pt-2 pb-2 w-fit md:w-full mt-8 h-fit lg:h-[61px] ${viewState ? "hidden" : "flex"} flex-col mdxl:flex-row justify-between items-center rounded-[48px] bg-[#F3F3F3] md:pr-[60px]`}>
